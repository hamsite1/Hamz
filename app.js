...
window.openSideMenu = () => { document.getElementById("sideMenu").classList.add("open"); document.getElementById("menuBackdrop").classList.add("open"); };
window.closeSideMenu = () => { document.getElementById("sideMenu").classList.remove("open"); document.getElementById("menuBackdrop").classList.remove("open"); };
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        if(data.length === 0 && !currentUser) {
            document.getElementById("syncVal").textContent = "أوفلاين 📴";
        }
    }, 3000);
});

// الاستماع لحدث عودة الإنترنت (Online)
...
// الاستماع لحدث انقطاع الإنترنت (Offline)
...
const FIREBASE_CONFIG = { apiKey: "AIzaSyDkIjEtlK2MOvk885bLL9gzDWeLL97sPVI", authDomain: "data1-25ab3.firebaseapp.com", databaseURL: "https://data1-25ab3-default-rtdb.europe-west1.firebasedatabase.app", projectId: "data1-25ab3", storageBucket: "data1-25ab3.firebasestorage.app", messagingSenderId: "1815036537", appId: "1:1815036537:web:a2d51718c72675fe784aa8" };

const BASE_APP_TITLE = "لوحة تسيير الاشتراكات - v12.9.18"; 
    ...
    auth.onAuthStateChanged(async (u)=>{ 
        ...
        if(u){ 
            ...
            adminListenerRef.on('value', async (roleSnap) => {
                ...
                if (!roleSnap.exists()) {
                    await db.ref("admins/" + u.uid).set({ role: "owner", email: u.email });
                    showToast("تم تتويجك كمالك! جاري الدخول...", "ok");
                    return; 
                }
                ...
                if(roleData === true) { 
                    try { 
                        roleData = { role: "owner", email: u.email }; 
                        await db.ref("admins/" + u.uid).set(roleData); 
                    } catch(writeErr) { roleData = { role: "owner", email: u.email }; } 
                } 
            ...
            currentRole = null; data = []; packagesData = []; window.render(); 
...
async function firebasePush() { 
  saveLocalBackup();
  if(!navigator.onLine) { 
    $("syncVal").textContent = "محفوظ محلياً 📴"; 
    return; 
  }
  try { 
    setSyncState("saving"); 
    ...
    showToast("خطأ في المزامنة مع السيرفر", "err"); 
    setSyncState("fail"); 
  } 
}
...
window.checkLicense = async () => {
    ...
    const expireDate = localStorage.getItem(LICENSE_EXPIRE_KEY);
    ...
    if (now > expiry) {
        ...
        errDiv.textContent = "❌ انتهت صلاحية اشتراكك! يرجى التواصل مع الإدارة للتجديد.";
